...
export default function Component() {
  const organizations = [
    ...
    },
    {
      ...
      description: "UNFCCC initiative that facilitates access to climate-resilient technologies in developing nations.",
      link: "#",
    },
    {
        ...
        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 border-t border-gray-200 pt-8">
          {organizations.map((org, index) => (
            <div key={index} className="flex flex-col gap-4 ">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{org.description}</p>
              <Link href={org.link} className="text-[#007AFF] hover:underline font-medium text-sm">
                Read here
              </Link>
            </div>
          ))}
